...
typedef struct
{
	...
	int16_t chassis_cycle_mode_1_speed; //底盘小陀螺模式0速度
	int16_t chassis_cycle_mode_2_speed; //底盘小陀螺模式0速度
	uint16_t chassis_output_max; //底盘最大输出
	uint16_t chassis_speed_max; //底盘最高速度
	float straight_rectify_coefficient;  //直线校正系数（校正的输出与当前输出的比值）（范围0~1）
}chassis_config_t;
