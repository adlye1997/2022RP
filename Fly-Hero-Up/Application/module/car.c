...
bool chassis_speed_limit = false;       //底盘速度限制
...
bool gimbal_yaw_lock = false;           //锁定yaw轴
bool gimbal_yaw_unlock = false;         //解锁yaw轴
bool gimbal_pitch_lock = false;         //锁定pitch轴
bool gimbal_pitch_unlock = false;       //解锁pitch轴
...
bool launcher_auto_shoot_on = false;    //自动射击开启
bool launcher_auto_shoot_off = false;   //自动射击关闭
  ...
  switch(rc.base_info->s2.status)
  {
    case up_R:
      ...
      car->ctrl_mode = KEY_CAR;
			chassis_speed_limit = true;
      break;
    case mid_R:
      car->move_mode_commond = machine_CAR;
      car->ctrl_mode = RC_CAR;
			chassis_speed_limit = false;
      break;
    case down_R:
      car->move_mode_commond = gyro_CAR;
      car->ctrl_mode = RC_CAR;
...
{
	if(rc.base_info->s1.value == 1)
	{
		auto_on = true;
	}
	else if(rc.base_info->s1.status == mid_R)
	{
		auto_off = true;
	}
	else if(rc.base_info->s1.value == 2)
	{
	}
...
void KEY_Z_status_check(car_t *car)
{
  ...
  {
    case down_K:
			gimbal_yaw_lock = true;
      break;
    case up_K:
			gimbal_yaw_unlock = true;
			break;
    case short_press_K:
      break;
    case long_press_K:
...
void KEY_C_status_check(car_t *car)
{
  ...
  {
    case down_K:
			gimbal_pitch_lock = true;
      break;
    case up_K:
			gimbal_pitch_unlock = true;
      break;
    case short_press_K:
      break;
    case long_press_K:
