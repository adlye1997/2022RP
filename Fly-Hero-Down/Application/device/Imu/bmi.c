...
float vx, vy, vz;
float ex, ey, ez;
float BMI_SUM[3] = {0};//{-0.4, -3.52 ,1.64};
													 ...
													 float *aax,float *aay,float *aaz)
{
  ...
  q3temp=q3; 
	...
	*yaw =  -atan2(2*(q1*q2 + q0*q3),q0*q0 +q1*q1-q2*q2 -q3*q3)*57.295773f;
	...
	vx_ = 2*(q1_*q3_ - q0_*q2_);//-sin(Pitch) cos(K,i)
	vy_ = 2*(q0_*q1_ + q2_*q3_);//sin(Roll)cos(Pitch) cos(K,j)
	vz_ = q0_*q0_ - q1_*q1_ - q2_*q2_ + q3_*q3_;//cos(Roll)cos(Pitch) cos(K,k)
	
	ex_ = (ay_*vz_ - az_*vy_) ;
	ey_ = (az_*vx_ - ax_*vz_) ;//切线方向加速度
	ez_ = (ax_*vy_ - ay_*vx_) ;
  ...
  q3temp_=q3_; 
	...
	norm_ = inVSqrt(q0_*q0_ + q1_*q1_ + q2_*q2_ + q3_*q3_);
	q0_ = q0_ * norm_;
	q1_ = q1_ * norm_;
	q2_ = q2_ * norm_;
	q3_ = q3_ * norm_;
